...
{
    public class ReportingServiceBD : IReportingService
    {
        private AbstractDbContext context;

        public ReportingServiceBD(AbstractDbContext context)
        {
        ...
        public List<StorageLoadViewModel> GetStorageLoad()
        {
            return context.Storages
                            .ToList()
                            .GroupJoin(
                                    context.Storage__GBIingridients
                                                .Include(r => r.GBIindgridient)
                                                .ToList(),
                                    stock => stock,
        ...
        public List<BuyerZakazesViewModel> GetBuyerZakazes(ReportingBindingModel model)
        {
            return context.Zakazes
                            .Include(rec => rec.Buyer)
                            .Include(rec => rec.GBIpieceOfArt)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new BuyerZakazesViewModel
                            {
        ...
        public void SaveGBIpieceOfArtPrice(ReportingBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.GBIpieceOfArts.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].GBIpieceOfArtNAme;
        ...
        public void SaveStorageLoad(ReportingBindingModel model)
        {
        ...
        public void SaveBuyerZakazes(ReportingBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetBuyerZakazes(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].GBIpieceOfArtName, fontForCells));
                table.AddCell(cell);
